...
export default function ShiftsPage() {
  ...
  const filteredShifts = useMemo(() => {
    return shifts.filter((shift) => {
      ...
      return (
        shift.exam?.examName?.toLowerCase().includes(searchLower) ||
    ...
    <div className="p-0 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm font-medium">Manage specific exam sittings and time slots.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-3 relative">
          ...
          <Input
            placeholder="Search by exam name, shift name, or code..."
            value={search}
            onChange={(e) => {
          ...
          <Table>
            <TableHeader className="bg-muted/30">
              <TableRow className="hover:bg-transparent border-t-0">
                <TableHead className="w-[80px] text-center font-bold text-[10px] uppercase tracking-widest pl-6">Code</TableHead>
                <TableHead className="font-bold text-[10px] uppercase tracking-widest">Shift Details</TableHead>
                <TableHead className="font-bold text-[10px] uppercase tracking-widest">Exam Reference</TableHead>
                <TableHead className="font-bold text-[10px] uppercase tracking-widest">Schedule</TableHead>
            ...
            <TableBody>
              {loading ? (
                ...
                paginatedShifts.map((shift) => (
                  <TableRow key={shift._id} className="hover:bg-muted/20 transition-colors group border-b">
                    <TableCell className="pl-6 text-center">
                      <code className="text-[10px] font-mono bg-primary/5 px-2 py-1 rounded text-primary font-bold border border-primary/10">
                        {shift.shiftCode || "N/A"}
                      </code>
                    </TableCell>
                    <TableCell>
                      <div className="font-bold text-sm tracking-tight">{shift.shiftName}</div>
                      <div className="text-[10px] text-muted-foreground mt-0.5 uppercase flex items-center gap-1">
                        <Clock className="h-3 w-3" /> {shift.startTime} - {shift.endTime}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="text-xs font-semibold">{shift.exam?.examName}</div>
                      <div className="text-[10px] text-muted-foreground uppercase">{shift.exam?.examYear}</div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <CalendarIcon className="h-3 w-3" />
                        {shift.date ? new Date(shift.date).toLocaleDateString() : "No Date"}
                      </div>
                    </TableCell>
                    <TableCell className="text-right pr-8">
                      <div className="flex justify-end gap-2">
                        <Link href={`/dashboard/shifts/${shift._id}`}>
                          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                            <Eye className="h-4 w-4" />
                          </Button>
                        </Link>
                        <Link href={`/dashboard/shifts/${shift._id}/edit`}>
                          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 shadow-xl">
                            ...
                            <DropdownMenuItem className="text-destructive focus:text-destructive">Archive Shift</DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
                    </TableCell>
        ...
        <CardFooter className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4 bg-muted/5 border-t border-muted/20">
          <div className="flex items-center gap-4">
            <div className="text-xs text-muted-foreground font-medium border-r pr-4 border-muted-foreground/20">
              Showing <b>{paginatedShifts.length}</b> of <b>{filteredShifts.length}</b> Results
            </div>
